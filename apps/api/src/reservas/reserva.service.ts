...
export class ReservaService {
  ...
  private async crearNotificacionReservaConfirmada(
    ...
    actividadNombre: string,
    email?: string,
    pago?: { fecha_pago?: Date | null; monto?: number; metodo?: 'MERCADOPAGO' | 'EFECTIVO' | 'TARJETA' | 'TRANSFERENCIA' },
  ) {
    if (!email) return;
  ...
  async createForEmail(createReservaDto: CreateReservaDto, email: string) {
    ...
    try {
      ...
      // No enviamos notificación de turno confirmado aún porque el pago presencial
      // se registra después de crear la reserva. El comprobante se genera cuando
      // se registra el pago usando PagosService.registrar().
      return {
  ...
  async crearReservaFijaForEmail(email: string, turnoInicialId: number, fechasString: string[]) {
    ...
    // Para presencial, crear directamente en CONFIRMADA
    // Para presencial, crear directamente en CONFIRMADA
    return this.crearReservaFija(usuario.paciente.id, turnoInicialId, fechasString, EstadoReserva.CONFIRMADA, EstadoReserva.CONFIRMADA);
  }
